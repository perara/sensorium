...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
	echo "remote-common.sh is an internal library; source it from remote workflow scripts." >&2
	exit 2
fi

sensorium_require_clean_env
remote_env_file="${SENSORIUM_REMOTE_ENV_FILE:-${sensorium_repo_root}/.env.remote}"

if [[ "${remote_env_file}" != "/dev/null" && -f "${remote_env_file}" ]]; then
...
remote_manifest_path="${REMOTE_MANIFEST_PATH:-${remote_repo_dir}/.sensorium-sync-manifest}"
remote_retry_count="${REMOTE_RETRY_COUNT:-8}"
remote_retry_delay_seconds="${REMOTE_RETRY_DELAY_SECONDS:-5}"
remote_rsync_checksum="${REMOTE_RSYNC_CHECKSUM:-0}"
...
remote_ssh_no_stdin() {
	ssh -n -p "${remote_port}" "${remote_ssh_opts[@]}" "${remote_target}" "$@"
}
...
remote_ssh_retry_no_stdin() {
	with_remote_retries remote_ssh_no_stdin "$@"
}

remote_rsync_to() {
	local rsync_args=(-az --delete)

	if [[ "${REMOTE_RSYNC_CHECKSUM:-${remote_rsync_checksum}}" == "1" ]]; then
		rsync_args+=(--checksum)
	fi
...
remote_sync_manifest_local() {
	PYTHONDONTWRITEBYTECODE=1 \
		python3 "${sensorium_repo_root}/scripts/tools/compute-sync-manifest.py" "${sensorium_repo_root}"
}

remote_sync_manifest_remote() {
	remote_ssh_retry "cat '${remote_manifest_path}' 2>/dev/null || true"
}

remote_write_sync_manifest() {
	local manifest="$1"
	remote_ssh_retry "printf '%s\n' '${manifest}' > '${remote_manifest_path}'"
}
...
remote_install_ipa_config() {
	...
	local remote_ipa_dir
	local remote_profile_yaml
	...
	remote_ipa_dir="$(remote_ssh_retry "cd '${remote_repo_dir}' && source ./scripts/lib/sensorium-common.sh && printf '%s' \"\${sensorium_libcamera_ipa_dir}\"")"
	if [[ -z "${remote_ipa_dir}" ]]; then
		echo "Failed to resolve remote libcamera IPA directory." >&2
		exit 2
	fi
	remote_profile_yaml="${remote_ipa_dir}/${sensorium_sensor}.yaml"
	remote_ssh_retry "sudo mkdir -p '${remote_ipa_dir}'"

	if [[ -f "${local_profile_yaml}" ]]; then
		remote_ssh_retry "sudo install -m 0644 '${remote_repo_dir}/config/ipa/simple/overrides/${sensorium_sensor}.yaml' '${remote_profile_yaml}'"
	else
		remote_ssh_retry "sudo install -m 0644 '${remote_repo_dir}/config/ipa/simple/imx-generic.yaml' '${remote_profile_yaml}'"
	fi
}
