...
echo "Running QEMU burn-in"
BURNIN_VERIFY_SYNC_REPAIR="${QEMU_VERIFY_SYNC_REPAIR:-1}" \
BURNIN_RUNTIME_STRESS_ITERATIONS="${QEMU_RUNTIME_STRESS_ITERATIONS:-3}" \
BURNIN_RUNTIME_STRESS_MODEL="${QEMU_RUNTIME_STRESS_MODEL:-models/runtime/rpi-multibus-burnin.yaml}" \
BURNIN_CAMERA_CYCLES="${QEMU_CAMERA_CYCLE_ITERATIONS:-2}" \
BURNIN_CAMERA_MATRIX_SENSORS="${QEMU_CAMERA_MATRIX_SENSORS:-imx708 imx219 imx477}" \
SKIP_SYNC=1 "${script_dir}/../remote/remote-burnin.sh"

echo
echo "QEMU burn-in complete."
