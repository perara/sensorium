...
raw_shift="${STREAM_RAW_SHIFT:-0}"
script_path="$(readlink -f "${BASH_SOURCE[0]}")"
script_dir="$(cd "$(dirname "${script_path}")" && pwd)"
repo_root="$(cd "${script_dir}/../.." && pwd)"
raw_converter="${repo_root}/tools/rgb24-to-rggb10"
...
RG10)
	...
	ffmpeg \
		...
		-i "${source_path}" \
		-an -sn -dn \
		-vf "fps=${fps},scale=${width}:${height}:force_original_aspect_ratio=decrease:force_divisible_by=2,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:black,format=rgb24" \
		-pix_fmt rgb24 \
		-f rawvideo - | \
	"${raw_converter}" --width "${width}" --height "${height}" --shift "${raw_shift}" | \
	v4l2-ctl \
		-d "${inject_device}" \
		--set-fmt-video-out="width=${width},height=${height},pixelformat=RG10" \
		--stream-out-mmap=4 \
		--stream-from=-
	;;
*)
	echo "Unsupported STREAM_PIXEL_FORMAT: ${stream_pixel_format}" >&2
	exit 2
	;;
esac
